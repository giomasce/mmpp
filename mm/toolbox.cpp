...
static void type_proving_helper_unwind_tree(const ParsingTree< SymTok, LabTok > &tree, ExtendedProofEngine< Sentence, SubstMapType, SymTok > &engine, const Library &lib, const std::unordered_map<LabTok, const Prover*> &var_provers) {
...
bool LibraryToolbox::type_proving_helper(const std::vector<SymTok> &type_sent, ExtendedProofEngine< Sentence, SubstMapType, SymTok > &engine, const std::unordered_map<SymTok, Prover> &var_provers) const
{
...
bool LibraryToolbox::type_proving_helper(const ParsingTree2<SymTok, LabTok> &pt, ExtendedProofEngine< Sentence, SubstMapType, SymTok > &engine, const std::unordered_map<LabTok, Prover> &var_provers) const
{
...
Prover LibraryToolbox::build_prover(const std::vector<Sentence> &templ_hyps, const Sentence &templ_thesis, const std::unordered_map<string, Prover> &types_provers, const std::vector<Prover> &hyps_provers) const
{
    ...
    return [=](ExtendedProofEngine< Sentence, SubstMapType, SymTok > &engine){
...
bool LibraryToolbox::proving_helper(const RegisteredProverInstanceData &inst_data, const std::unordered_map<string, Prover> &types_provers, const std::vector<Prover> &hyps_provers, ExtendedProofEngine< Sentence, SubstMapType, SymTok > &engine) const {
            ...
            if (!unifications.empty()) {
                for (auto &unification : unifications) {
                    ...
                    for (auto &float_hyp : ass.get_float_hyps()) {
                        ...
                        ExtendedProofEngine< Sentence, SubstMapType, SymTok > engine(*this);
...
string test_prover(Prover prover, const LibraryToolbox &tb) {
    ExtendedProofEngine< Sentence, SubstMapType, SymTok > engine(tb);
...
{
    return [=](ExtendedProofEngine< Sentence, SubstMapType, SymTok > &engine)->bool {
