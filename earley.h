...
struct EarleyTreeItem {
    LabType label;
    std::vector< EarleyTreeItem< LabType > > children;
};
...
struct EarleyState {
    ...
    EarleyTreeItem< LabType > get_tree(const std::vector< std::vector< EarleyState > > &state) const {
        EarleyTreeItem< LabType > ret;
...
template< typename SymType, typename LabType >
EarleyTreeItem< LabType > earley(const std::vector<SymType> &sent, SymType type, const std::unordered_map<SymType, std::vector<std::pair<LabType, std::vector<SymType> > > > &derivations)
{
    ...
    for (size_t j = 0; j < state[sent.size()].size(); j++) {
        ...
        if (cur_state.initial == 0 && cur_state.current == cur_state.der->size() && cur_state.type == type) {
            EarleyTreeItem< LabType > tree = cur_state.get_tree(state);
            return tree;
        }
    }
    EarleyTreeItem< LabType > ret;
